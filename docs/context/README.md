...
- C# uses Tree-sitter (`tree-sitter-c-sharp`) with the same contract as Java: namespaces, types, members, control flow and handlers become nodes; `#region` and other preprocessor lines are kept as leaf `preproc` nodes but never form regions; expressions, lambdas and query syntax stay indivisible; syntax errors make the file opaque; calls, `new`, `goto` and identifiers are lexical candidates only.
