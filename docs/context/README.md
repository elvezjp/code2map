...
- C# uses Tree-sitter (`tree-sitter-c-sharp`) with the same contract as Java: namespaces, types, members, control flow and handlers become nodes. `#if`/`#elif`/`#else` blocks keep the declarations they guard as children (both sides indexed as written, nothing evaluated); `#region` and other directive lines are leaf `preproc` nodes that never form regions. Shared `case` labels stay in one branch header. Expressions, lambdas and query syntax stay indivisible; syntax errors make the file opaque; calls, `new`, `goto` and identifiers are lexical candidates only.
