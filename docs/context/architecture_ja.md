...
Python ASTは実行環境に依存するためPythonの版を記録する。JavaではTree-sitter本体と文法の版も記録する。原文のバイト列、文字コード、相対パス、実行環境とアダプターの実装版、トークナイザーの語彙・設定、分割設定を固定すれば、出力はバイト単位で再現する。独自アダプターとカウンターも決定論的でなければならない。
