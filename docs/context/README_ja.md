...
- C#はTree-sitter（`tree-sitter-c-sharp`）を使い、Javaと同じ契約です。名前空間・型・メンバー・制御構文・例外ハンドラーをノードにし、`#region`などの前処理行は葉の`preproc`ノードとして位置だけ残します（範囲にはしません）。式・ラムダ・クエリ構文は分割しません。構文エラーはファイル全体を`opaque`にします。呼出・`new`・`goto`・識別子は字句候補にすぎません。
