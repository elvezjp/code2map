...
- C#はTree-sitter（`tree-sitter-c-sharp`）を使い、Javaと同じ契約です。名前空間・型・メンバー・制御構文・例外ハンドラーをノードにします。`#if`／`#elif`／`#else`のブロックは囲んだ宣言を子に持ち、条件の両側を書かれたまま索引化します（評価はしません）。`#region`などのディレクティブ行は葉の`preproc`ノードで、範囲にはしません。共有する`case`ラベルは一つの分岐ヘッダーに残します。式・ラムダ・クエリ構文は分割しません。例外は式形式メンバー内のswitch式で、armごとに分割します。構文エラーはファイル全体を`opaque`にします。呼出・`new`・`goto`・識別子は字句候補にすぎません。
