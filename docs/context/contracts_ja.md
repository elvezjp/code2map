...
各ソースに全文を覆うfileノードが1つある。兄弟ノードは重複しない。子は親の範囲に含まれる。子ノードの間の空白やコメントを木に登録する必要はなく、分割器が隙間を保存する。分割器は子ノードの境界で範囲を分けるとき、境界を行の先頭・末尾に揃える（字下げの後で始まる子や行末コメントの前で終わる子でも、隣の範囲に行の断片を残さない。同じ行を2つの子が共有する場合は先の子がその行を持つ）。
