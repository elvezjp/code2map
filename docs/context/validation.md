...
Context-engine output samples are recorded under `context/` for each language in `docs/examples/v0.5.0/`.
...
The current lockfile pins Tree-sitter 0.26.0, Java grammar 0.23.5 and C# grammar 0.23.5 (as of the C# adapter). Earlier records (2026-09-05 to 09-08) were produced with the previous lock (Tree-sitter 0.25.2, Java grammar 0.23.5); the fresh wheel installation additionally exercised Tree-sitter 0.26.0 with Java grammar 0.23.5. This is not an exhaustive dependency-version compatibility claim.
