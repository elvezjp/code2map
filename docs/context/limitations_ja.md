...
構造は`tree-sitter-c-sharp`の文法から取る。ブロック形式の名前空間はスコープになる。ファイルスコープの`namespace X;`は葉の`import`ノードとして記録し、その下の名前を修飾しない。クラス・構造体・インターフェース・レコード・列挙型・メソッド・コンストラクター・プロパティ・インデクサー・演算子・ローカル関数がノードになり、プロパティのアクセサーはプロパティ配下の分岐になる。制御構文（if／else・各種ループ・switch文と式・try／catch／finally・using・lock・checked・unsafe・fixed）は本体に沿って分割する。`do`は末尾条件を失わないよう分割不能として扱う。ラムダ・匿名メソッド・クエリ式・属性は分割しない。別ファイルの`partial`型は別ノードで、結合しない。前処理行（`#region`・`#if`等）は葉の`preproc`ノードで、条件付きコンパイルは評価しない（無効な側も書かれたまま索引化する）。呼出・オブジェクト生成・`goto`・識別子は字句候補で、オーバーロード・拡張メソッド・型の解決は行わない。構文エラーはファイル全体を`opaque`にする。索引にはTree-sitter本体とC#文法の版を記録する。
