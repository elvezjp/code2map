...
構造は`tree-sitter-c-sharp`の文法から取る。ブロック形式の名前空間はスコープになり、配下の名前を修飾する。ファイルスコープの`namespace X;`は葉の`import`ノードとして記録し、何も修飾しない。クラス・構造体・インターフェース・レコード・列挙型・メソッド・コンストラクター・デストラクター・プロパティ・インデクサー・演算子・ローカル関数がノードになり、プロパティのアクセサーはプロパティ配下の分岐になる。式形式メンバーにはブロックが無いが、矢印節の中のswitch式はarmごとに分割する。制御構文（if／else・各種ループ・switch文と式・try／catch／finally・using・lock・checked・unsafe・fixed）は本体に沿って分割し、`else`は条件をヘッダーに残す。`do`は末尾条件を失わないよう分割不能として扱う。switch文で本体を持たないラベルだけの節（`case 1:`の直後に`case 2:`）は、本体を持つ節の分岐に併合し、分割後もすべてのラベルがその分岐のヘッダーに残る。ラベル付き文は`label`ノードで、`goto`は`jump`の候補になる。

前処理ディレクティブは評価しない。`#if`／`#elif`／`#else`のブロックは、囲んでいる宣言や文を子に持つ`preproc`ノードで、条件の両側を書かれたまま索引化し分割する。`#region`・`#define`・`#pragma`などの行は葉の`preproc`ノードで、位置は残るが範囲にはならない。ラムダ・匿名メソッド・クエリ式・属性は分割しない。別ファイルの`partial`型は別ノードで、結合しない。呼出・オブジェクト生成・`goto`・識別子は字句候補で、オーバーロード・拡張メソッド・型の解決は行わない（オーバーロードは候補を全部残す）。構文エラーはファイル全体を`opaque`にし、`CSHARP_PARSE_ERROR`の診断を出す。索引にはTree-sitter本体とC#文法の版を記録する。先頭のBOM（U+FEFF）は文法が空白として受け入れるため構文エラーにならないが、既定の`utf-8`では原文の1文字目として残り、位置が1文字ずれ、payloadにも含まれる。除きたい場合は`--encoding utf-8-sig`を指定する（`text_sha256`だけが変わる）。
