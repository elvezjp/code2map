...
前処理ディレクティブは評価しない。`#if`／`#elif`／`#else`のブロックは、囲んでいる宣言や文を子に持つ`preproc`ノードで、条件の両側を書かれたまま索引化し分割する。`#region`・`#define`・`#pragma`などの行は葉の`preproc`ノードで、位置は残るが範囲にはならない。ラムダ・匿名メソッド・クエリ式・属性は分割しない。別ファイルの`partial`型は別ノードで、結合しない。呼出・オブジェクト生成・`goto`・識別子は字句候補で、オーバーロード・拡張メソッド・型の解決は行わない（オーバーロードは候補を全部残す）。構文エラーはファイル全体を`opaque`にし、`CSHARP_PARSE_ERROR`の診断を出す。索引にはTree-sitter本体とC#文法の版を記録する。
