...
1. **第1フェーズ**: エラーメッセージの改善 — パース失敗時に原因と発生箇所を正しく伝える（v0.2.0）
2. **第2フェーズ（要検討）**: パーサーライブラリの置き換え — Java 8+構文を根本的にサポートする（v0.3.0）
...
本修正はパーサーの動作変更を含むため、バージョンを v0.1.2 → v0.2.0 に上げる。
...
### Task 1: 現行バージョン（v0.2.0）のスナップショット保存

1. `versions/v0.2.0/` ディレクトリを作成
2. 以下をコピー:
   - `code2map/` （パッケージ全体）
   - `tests/`
   - `pyproject.toml`
...
1. `pyproject.toml` の version を `"0.3.0"` に更新
2. `code2map/__init__.py` の `__version__` を `"0.3.0"` に更新
3. `CHANGELOG.md` に v0.3.0 のエントリを追加
...
- [ ] `versions/v0.2.0/` に v0.2.0 コードのスナップショットが保存されている
...
- [ ] ネストクラスが正しく抽出されること
- [ ] `pyproject.toml` の version が `"0.3.0"` に更新されている
- [ ] `code2map/__init__.py` の `__version__` が `"0.3.0"` に更新されている
- [ ] `CHANGELOG.md` に v0.3.0 のエントリが追加されている
- [ ] PR が作成されレビュー依頼されている
