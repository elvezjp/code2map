...
- `build` は入力ファイルの拡張子（`.py`, `.java`）に基づいて言語を判定します。共通エンジン（`index`）は `.sql` `.pks` `.pkb` `.pls` `.plsql` `.py` `.java` `.cs` を対象にし、それ以外のファイルは索引に含めません
...
- 共通エンジンの `index.json` は**入力ファイルの原文全体**を保持し、`pack.json` の各 packet にも原文の断片が含まれます。これらは解析対象のソースコードそのものとして取り扱ってください（記録されるパスは入力からの相対パスで、絶対パスや時刻は含みません）

### 依存関係

- 依存関係は定期的に脆弱性スキャンを実施しています
- `uv sync`で、`uv.lock`に記録された依存関係のバージョンに環境を同期できます
...
**補足**: v0.2.1 まではリポジトリ内に旧バージョンのスナップショット（`versions/`）を保持しており、それらの lockfile に対しても Dependabot アラートが発生していました。同ディレクトリは 0.3.0 で削除済みのため（バージョン管理は git tag に移行、README 参照）、以降のアラートはリポジトリルートの manifest に対してのみ発生します。削除前のディレクトリ由来のアラートが残っている場合は、影響を確認のうえ Dismiss して close します。
...
- 本ツールは静的解析のみを行い、コードの実行は行いません。索引化・分割の際にネットワークアクセスや外部サービス（LLM等）の呼び出しも行いません
