...
0.4.0で、原文全体を先に索引化し、予算に応じて文脈付きの入力を組み立てる共通エンジンを追加しました。PL/SQL・Python・Java・C#、およびこれらが混在するディレクトリに対応します。既存の`build`コマンドと出力形式も継続して利用できます。
...
- `main` ブランチには次バージョンの変更を [CHANGELOG_ja.md](CHANGELOG_ja.md) の次バージョンの見出し（日付の位置に「未リリース」と書く）の下に蓄積します
